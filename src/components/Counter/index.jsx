...
const {theme} = React.useContext(ThemeContext)
  ...
  React.useEffect(() => {
    api
      .get("all")
      .then((res) => setCountry(res.data) || console.log(res.data.flag))
      .catch((err) => console.log(err.message));
    
  }, []);

  return (
    <section className={`country-section site--${theme}`}>
      <div className="container">
        <form className="country-section__form">
          <input
            className={`country-section__form--input site--${theme}`}
            type="search"
            name="country_name"
            placeholder="Search for a country…"
          />
          <select className={`country-section__form--select site--${theme}`}>
